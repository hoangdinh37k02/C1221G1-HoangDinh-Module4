...
public class BlogRestController {
    ...
    @GetMapping("/list")
    public ResponseEntity<Page<Blog>> getPageBlog(@RequestParam Optional<String> name,  @PageableDefault(value = 2)Pageable pageable){
        String nameVal = name.orElse("");
        Page<Blog> blogPage = this.iBlogService.getBlogByName(nameVal,pageable);
...
//        blog.setBlogType(blogType);
    ...
    public ResponseEntity<List<FieldError>> createBlog2(@Validated @RequestBody BlogDto blogDto,
                                                       BindingResult bindingResult){
        if (bindingResult.hasFieldErrors()){
            return new ResponseEntity<>(bindingResult.getFieldErrors(), HttpStatus.BAD_REQUEST);
        }
